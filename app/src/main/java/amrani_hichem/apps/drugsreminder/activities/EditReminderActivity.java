...
import amrani_hichem.apps.drugsreminder.elements.Drug;
import amrani_hichem.apps.drugsreminder.elements.IReminder;
import amrani_hichem.apps.drugsreminder.libraries.ElementsLibrary;
import amrani_hichem.apps.drugsreminder.elements.Reminder;
import amrani_hichem.apps.drugsreminder.preferences.Preferences;
import amrani_hichem.apps.drugsreminder.views.ScrollDisabledListView;
import amrani_hichem.apps.drugsreminder.adapters.ImprovedSimpleAdapter;
...
public class EditReminderActivity extends EditElementActivity<IReminder> {
	...
	protected void setupViews() {
		...
		checkedMealIDs = new ArrayList<>();
		drugListItems = new ArrayList<>();
		...
		findViewById(R.id.edit_reminder_add_drug).setOnClickListener(view -> {
			...
			if (addDrugsNumber == 0)
				...
				List<Map<String, Object>> addDrugItems = new ArrayList<>(addDrugsNumber);
				List<Integer> addDrugIDs = new ArrayList<>(addDrugsNumber);
				for (int addDrugID = 0; addDrugID < ElementsLibrary.drugsNumber(); addDrugID++) {
					...
					Map<String, Object> addDrugItem = new HashMap<>(2);
	...
	protected void loadViews(IReminder reminder) {
		...
		drugIDs = new ArrayList<>(aReminder.getDrugIDs());
	...
	protected void onSaveInstanceState(@NonNull Bundle outState) {
		...
		outState.putIntegerArrayList("mealIDs", new ArrayList<>(checkedMealIDs));
		outState.putIntegerArrayList("drugIDs", new ArrayList<>(drugIDs));
		outState.putStringArrayList("usageDosages", new ArrayList<>(getUsageDosages()));
